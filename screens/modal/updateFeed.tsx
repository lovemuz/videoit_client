...
export default function UpdateFeed({
  ...
  const postCopy = route.params.post;
  const [content, setContent] = useState(postCopy?.content);
  const [url, setUrl]: any = useState(postCopy?.url);
  //사진은 바꿀수 없도록
  const [urlFormData, setUrlFormData] = useState(null);
  const [thumbnailFormData, setThumbnailFormData] = useState(null);
  const [free, setFree] = useState(postCopy?.lock);
  const [adult, setAdult] = useState(postCopy?.adult);
  const [contentSecret, setContentSecret] = useState(postCopy?.rContentSecret);
  const [price, setPrice] = useState(postCopy?.cost);
  const [onlyMember, setOnlyMember] = useState(
    postCopy?.cost === -1 ? true : false,
  );
  ...
  const [step, setStep] = useState(postCopy?.step);
  const [urlType, setUrlType] = useState(
    postCopy?.type === POST_TYPE.POST_VIDEO ? "video" : "image",
  );
                  ...
                  onPress={async e => {
                    ...
                    if (urlFormData) {
                      if (urlType === "video") {
                        await axios
                          .post(
                            `${serverURL}/etc/addImgSecureImage`,
                            urlFormData,
                            {
                    ...
                    await api
                      ...
                      .then(res => {
                        if (res.data.status === "true") {
                          Alert.alert(
                            country === "ko"
                              ? "게시글이 수정 되었습니다. 새로고침 해주세요."
                              : country === "ja"
                              ? "投稿が修正されました。リロードしてください。"
                              : country === "es"
                              ? "La publicación ha sido editada. Porfavor refresca."
                              : country === "fr"
                              ? "Le message a été édité. Rafraichissez, s'il vous plait."
                              : country === "id"
                              ? "Postingan telah diedit. Harap segarkan."
                              : country === "zh"
                              ? "该帖子已被编辑。请刷新。"
                              : "The post has been edited. Please refresh.",
                          );
                          //수정 된거 렌더링
                          navigation.goBack();
                        } else {
              ...
              <ScrollView
                ...
                {/*
                  {props.item?.type === POST_TYPE.POST_IMAGE ? (
                            <FastImage
                              source={{
                                uri: `https://api.nmoment.live/secure/post/image/${props.item?.id}`,
                                headers: {
                            ...
                            <>
                              <FastImage
                                source={{
                                  uri: `https://api.nmoment.live/secure/post/video/${props.item?.id}`,
                                  priority: FastImage.priority.normal,
                                  headers: {
                                    authorization: `Bearer ${accessToken}`,
                                    refreshToken: `Bearer ${refreshToken}`,
                    ...
                    {urlType === "video" ? (
                      <TouchableOpacity
                        ...
                        <WebView
                          ...
                          source={{
                            uri: `https://nmoment.live/postwebview/${postCopy?.id}?accessToken=${accessToken}&controls=true`,
                          }}
                      ...
                      <FastImage
                        source={{
                          uri: `https://api.nmoment.live/secure/post/image/${postCopy?.id}`,
                          headers: {
                            authorization: `Bearer ${accessToken}`,
                            refreshToken: `Bearer ${refreshToken}`,
                          },
